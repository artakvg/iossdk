...
xcodebuild -target ${PRODUCT_NAME} -configuration ${CONFIGURATION} -sdk iphonesimulator -arch i386, x86_64 BUILD_DIR="${BUILD_DIR}" BUILD_ROOT="${BUILD_ROOT}"
...
cp -R "${BUILD_DIR}/${CONFIGURATION}-iphoneos/include/${PRODUCT_NAME}/Forkize.h" "${UNIVERSAL_OUTPUTFOLDER}/"
cp -R "${BUILD_DIR}/${CONFIGURATION}-iphoneos/include/${PRODUCT_NAME}/UserProfile.h" "${UNIVERSAL_OUTPUTFOLDER}/"
